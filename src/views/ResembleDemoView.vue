...
const compareImg = () => {
  ...
  if (inputImgs.length === 2) {
    ...
    resemble(image2data(inputImgs[0]))
        .compareTo(image2data(inputImgs[1]))
        .ignoreLess()
        .onComplete(data => {
          console.log(data)
          // 显示对比结果
          output.value = `相似度：${100 - data.misMatchPercentage}% (识别速度：${data.analysisTime}ms)  比较width:${canvas.width}px  比较height:${canvas.height}px`;
        });
  } else {
    alert("选择图片啊？草~凸(艹皿艹 )")
  }
